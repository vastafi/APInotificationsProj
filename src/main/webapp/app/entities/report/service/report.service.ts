...
type RestOf<T extends IReport | NewReport> = Omit<T, 'generatedDate'> & {
...
export class ReportService {
  ...
  protected convertDateFromClient<T extends IReport | NewReport | PartialUpdateReport>(report: T): RestOf<T> {
    return {
      ...report,
      generatedDate: report.generatedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restReport: RestReport): IReport {
    return {
      ...restReport,
      generatedDate: restReport.generatedDate ? dayjs(restReport.generatedDate) : undefined,
    };
  }
